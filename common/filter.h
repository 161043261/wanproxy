...
public:
    ...
    virtual void flush(int flg) {
        if (recipient_) recipient_->flush(flg);
    }
};
...
public:
    BufferedFilter(const LogHandle &log) : log_(log) {
        flushing_ = 0;
        flush_flags_ = 0;
    }
};
...
public:
    ...
    void prepend(Filter *f) {
        Filter *act = (nodes_.empty() ? holder_ : nodes_.front());
        if (f && act) nodes_.push_front(f), chain(f), f->chain(act);
    }

    void append(Filter *f) {
        Filter *act = (nodes_.empty() ? this : nodes_.front());
        if (f && act) nodes_.push_front(f), act->chain(f), f->chain(holder_);
    }

    virtual void flush(int flg) {
        if (nodes_.empty()) chain(holder_);
        Filter::flush(flg);
    }
};
