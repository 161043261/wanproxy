...
template<typename T, int N = STANDARD_RING_BUFFER_CAPACITY>
class RingBuffer {
private:
...
template<typename T, int N>
int RingBuffer<T, N>::write(const T &src) {
    unsigned char *r;
    unsigned char *w;
    int s, t, n1, n2;
...
public:
    ...
    void wakeup() {
        if (ring.is_empty()) pthread_cond_signal(&ready);
    }
};

template<typename T>
WaitBuffer<T>::WaitBuffer() {
...
template<typename T>
int WaitBuffer<T>::write(const T &src) {
